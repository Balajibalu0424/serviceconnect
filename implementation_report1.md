...
| Forgot-password flow | The request/reset architecture and token table already existed, but the reset link used `/#/reset-password?token=...`, which broke on direct load for the same hash/query reason and made the flow effectively unusable. The route also lacked compatibility handling for legacy reset links. |
...
- Pending final production deploy/retest in this session.
